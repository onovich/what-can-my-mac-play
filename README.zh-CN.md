...
样本目录仍来自仓库内的时点调研，属于静态数据。Cloudflare Worker 已包含经过测试的 Steam 应用列表边界，以及带短周期滥用防护、默认关闭的游戏库查询边界；线上游戏库功能会继续关闭，直到全局每日硬预算、获准测试账号与最终隐私说明准备完成。项目目前没有用户账户系统，也没有自动兼容性数据接入管线。
