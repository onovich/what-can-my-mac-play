...
The sample catalog remains static and comes from the repository's point-in-time research. A Cloudflare Worker now provides a tested Steam app-list boundary and a fail-closed owned-games boundary with short-window abuse limits and a strongly consistent 80,000-call daily budget. Live owned-games lookup remains disabled until the production key, approved test accounts, and final privacy copy are ready. There is no user account system or automated compatibility-data ingestion pipeline.
