...
- 本批原计划核验 10–15 款候选；实际核验中发现启动链、存档、新版本与商店版本映射等问题，未通过者留在内部研究文档，不为凑到 30 款而准入。
- 依据与暂缓原因见 `docs/research/catalog-expansion-2026-09-06.md`；参考标识 `store:<appId>:2026-09-06` / `codeweavers:<appId>:2026-09-06` 对应该表的 Steam API 和 CodeWeavers 条目。
- 增加目录唯一性、商店链接、支持记录关联及新增别名回归测试；搜索结果计数测试不再写死目录总数。

## 2026-09-06 方案名称一致性补全（历史）

- 名称独立为 `src/content/routeNames.ts`，以完整 `Runner` × `Locale` 类型约束；取消漏配时显示内部字段的兜底，新增方案或语言时必须补齐映射。
- 列表、详情、方案选择、返回首选以及历史购买组件共用映射。删除重复名称；配置难度仍属于配置说明，不拼在工具名上。
- 增加中英文方案切换回归测试。虚拟机与 Whisky 的内部名称不代表准入，两者仍不出现在方案选择中。

## 2026-09-05 运行方案名称统一（历史）

- 移除逐游戏手写的推荐 `title`；首页与详情均通过 `runner` 字段的共享本地化查找生成方案名称，统一为 CrossOver、Mac 版等。
- 游戏特别版等版本信息留在游戏标题；系统、单人、购买限制迁移到独立 `note`，不再与渠道和运行方案混写。没有改变支持范围或准入结论。
- 中英文一致性与限制保留的回归测试通过；完整检查共 125 项测试通过。

## 2026-09-05 非原生目录扩充与搜索改进（历史）

- 目录从 12 增至 16 款，新增《天际特别版》《女神异闻录 5 皇家版》《死亡之门》《黑暗之魂 3》。前 3 款进入精选；黑魂 3 支持查询但不占首页优先位。原生 Mac 游戏继续只作搜索兜底。
- 首页从 3 扩至 6 款，通过独立编辑顺序优先展示支持好、限制少的游戏并覆盖不同玩法。P5R 的推荐范围为 macOS 15+，首页与详情均注明；该范围不是发行商官方 Mac 最低配置。
- 搜索忽略大小写、空格、标点和全角字符，完整名称/别名优先于前缀及部分匹配。补充老滚 5、P5R、DS3、老头环等常用简称；不会把不同版本的支持合并。
- 新增依据见 `docs/research/catalog-expansion-2026-09-05.md`。教程和 Steam 库接入维持低优先级。

## 2026-09-05 首页精选与完整搜索（历史）
...
- `/library` 只提示未开放并返回搜索，不再展示公开/私有/空库模拟、未来流程或开发术语。
- 中英同步精简；浏览器语言检测、英文兜底和上次选择记忆不变。未新增实测或升级购买保证；Witcher 新购暂缓、Ori 版本要求、Sekiro 外屏处理、Elden 单人限制保留。

## 2026-09-04 决策型体验更新（历史记录）

- 玩家入口改为“我们的结论 → 首选方案 → 直接怎么做 → 最多两项必要条件”，不再展示原帖、作者转述、未知字段清单或兼容性/可信度双评分。研究和评分数据留在内部，不伪造新的实测或提高可信度。
- 支持清单采用明确准入：没有游戏/方案支持项，显示“当前清单不支持”并明确不推荐投入；这不是“技术上绝对无法运行”的断言。跨运行器、商店与架构不继承支持。
- 6 款游戏已给出逐款编辑判断；CrossOver 是已纳入的首选路线，Noita / Portal 2 为推荐方案，其余带必要条件。Elden Ring 明确不推荐为 Mac 联机购买；Witcher 3 明确新购等待已公告的配置变更复核。
- 首页列表、游戏详情和游戏库演示统一显示建议；移除首页尚未影响结果的配置预览。原始报告组件不再进入玩家页面，源数据仍可审计。
- 规则与内部索引见 [决策型产品口径](docs/product-decision-policy.md)。下一步应在这一口径下扩充可准入的方案和游戏，而非重新把研究负担交给玩家。

## 2026-09-04 产品范围更新（优先于下方历史规划）

- P0：单款游戏购买前检查；P1：个人 Steam 已购库分析；P2：发现型推荐。
- 纳入原生 Apple silicon、Intel Mac / Rosetta、CrossOver、Porting Kit；Wine 与 Sikarugir 作为高级配置候选。Whisky 仅保留历史参考。
- 排除虚拟机、云游戏、远程串流；VM 报告不得进入当前评分、可信度与推荐。GPTK 是开发者工具，D3DMetal / DXVK / DXMT 是图形后端。
- 已实现首页购买前入口、已发布游戏详情的双语方案比较、费用类别、配置门槛、购买检查清单。替代方案没有对应游戏证据时显示未知，不继承 CrossOver 分数。
- 数据覆盖：50 款研究清单、10 款评分回放、6 款当前发布的前端样本；不是 50 款已实测游戏。所有当前购买结论均为证据不足，不作购买保证。
- 调研依据：[非虚拟机购买决策调研](docs/research/non-vm-purchase-decision.md)。没有安装运行器或新增游戏实测。
- 已补齐 6 款发布样本的商店平台快照（2026-09-04）：当前均未标注 macOS 支持；Portal 2 旧 Mac 要求与当前平台标记分开展示；Witcher 3 显示 2026-09-29 配置变更提醒。Noita 的 Porting Kit 资料标注为 2023 年 GOG 配方，不作 Steam 验证。所有资料均带来源、核对日期；配方另带原文日期，不参与兼容性评分。详见[游戏级购买证据调研](docs/research/sample-game-purchase-evidence.md)。
- 已接入 3 条历史第一手自述（覆盖 Elden Ring / Noita 两款游戏）：分开呈现作者、商店版本、已知环境、缺失字段和发表/测试/核对日期；按游戏与运行方案隔离。Noita 自述和此前配方是同一来源，不增加独立证据数。未加入评分、未提高购买保证。详见[运行记录调研](docs/research/sample-runtime-reports.md)。
- 后续增量：新增 2025-07-21 的 Sekiro / M4 Pro / CrossOver Preview 第一手帖子，当前合计 4 条记录、覆盖 3 款游戏。明确标注外接屏全屏打开图形设置的条件性崩溃及作者自述的窗口模式绕行；不把预览版结果推广到正式版，不记录未经验证的平均帧率。各报告新增启动、游玩、存档、通关、联机、反作弊六项覆盖状态，未报告不等于成功或失败。仍未找到可支持当前配置购买保证的完整新证据，见[较新运行记录复核](docs/research/recent-runtime-review.md)。
- 下一步：逐款补齐具体商店版本、芯片、内存、macOS、引擎与图形后端版本及反作弊/联机/存档证据；之后再提升购买建议等级。Steam 实时功能继续关闭。
...
当前工程进度：Vite、React 与 TypeScript 前端已部署到 `macplay.onovich.com`，支持中英文语言记忆、设备画像、50 款研究样本、兼容性与可信度双评分及证据追溯。Cloudflare Worker 已实现 Steam 官方 App List 的服务端安全边界，以及默认关闭的 GetOwnedGames 边界；后者包含接口总量与 SHA-256 匿名库维度的短周期限流，以及按 UTC 日强一致执行的 80,000 次硬预算。生产 Steam Secret、最终隐私文案和真实账号验证尚未完成，因此实时同步暂未启用。用户系统与前端 Steam 库连接尚未实现。
...
- [x] 实现默认关闭的 GetOwnedGames 服务端连接器、请求边界、短周期双层限流、每日 80,000 次强一致硬预算和自动化测试；待生产密钥、最终隐私文案与真实账号分支验证完成后再启用；
...
- [x] 完成首页和可搜索的示例浏览流程；
- [x] 完成基于 Steam App ID 稳定 URL 的中英文游戏详情页，并保留评分边界、环境缺口与证据外链；
- [x] 完成 My Library 中英文可点击低保真流程，覆盖公开库、私密/不可访问、空库三种独立状态；实时连接前不接收 SteamID；
- [x] 部署可点击的响应式 SPA 原型；SSR/预渲染仍待 SEO 阶段评估；
- [ ] 加方法论、来源和免责声明页；
